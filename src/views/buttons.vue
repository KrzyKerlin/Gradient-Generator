...
<script setup>
  ...
  const buttons = ref([
    ...
    { label: 'Move', class: 'move-button', css:`
    .move-button { background-color: #890c82; border: none; border-radius: 5px; transition: transform 0.2s ease; box-shadow: 0 10px 20px rgba(0,0,0,0.2); }
    .move-button:hover { transform: translateY(-10px); box-shadow: 0 15px 25px rgba(0,0,0,0.2); }` },
    { label: 'Scale', class: 'scale-button',  css:`
    .scale-button { background-color: #28a745; color: white; transition: transform 0.3s ease;}
    .scale-button:hover { transform: scale(1.1); } }
    .shadow-button:hover { box-shadow: 0px 8px 12px rgba(0, 0, 0, 0.2); }` },
    { label: 'Dashed', class: 'dashed-button',  css:`
    .dashed-button { background: white; color: #6a11cb; border: 2px dashed #6a11cb; }
    .dashed-button:hover { background: #6a11cb; color: white; }` },
    { label: 'Glow', class: 'glow-button',  css:`
    .glow-button { background-color: #6610f2; color: white; box-shadow: 0 0 5px #6610f2, 0 0 15px #6610f2, 0 0 30px #6610f2, 0 0 50px #6610f2; transition: box-shadow 0.3s ease; }
    .glow-button:hover { box-shadow: 0 0 20px #6610f2, 0 0 40px #6610f2, 0 0 60px #6610f2, 0 0 100px #6610f2; }` },
    { label: '3D', class: 'button-3d', css:`
    .button-3d { background-color: #17a2b8; color: white; box-shadow: 0 5px #138496, 0 10px #117a8b; transition: box-shadow 0.3s ease; }
    .button-3d:hover { box-shadow: 0 2px #138496, 0 5px #117a8b; }` },
    { label: 'Outline', class: 'outline-button', css:`
    .outline-button { background-color: transparent; border: 2px solid #007BFF; color: #007BFF; transition: background-color 0.3s ease; }
    .outline-button:hover { background-color: #007BFF; color: white; }` },
    { label: 'Neon', class: 'neon-button', css:`
    .neon-button { background-color: #111;color: #0f0; border: 2px solid #0f0; border-radius: 5px; padding: 12px 24px;text-shadow: 0 0 5px #0f0, 0 0 10px #0f0, 0 0 20px #0f0; transition: all 0.3s ease-in-out; }
    .neon-button:hover { background-color: #0f0; color: #111; text-shadow: 0 0 10px #0f0, 0 0 20px #0f0, 0 0 40px #0f0; }` },
    { label: 'Ribbon', class: 'ribbon-button', css:`
    .ribbon-button { background-color: #ff4757; color: white; border: none; border-radius: 5px; padding: 12px 24px;position: relative; overflow: hidden; transition: all 0.3s ease-in-out; }
    ...
    { label: 'Wavy', class: 'wavy-button', css:`
    .wavy-button { background-color: #1e90ff; color: white; border: none; border-radius: 5px; padding: 12px 24px; position: relative; overflow: hidden; transition: all 0.3s ease-in-out; }
    ...
    { label: 'Slide', class: 'slide-button', css:`
    .slide-button { display: inline-block; font-weight: 700; color: white;border: 3px solid rgb(252, 70, 100); cursor: pointer; position: relative; background-color: transparent;text-decoration: none; overflow: hidden; z-index: 1; }
    ...
    { label: 'Rotate', class: 'rotate-button', css:`
    .rotate-button { background-color: #e83e8c; color: white; border: none;border-radius: 5px; padding: 12px 24px; transition: transform 0.3s ease; }
    .rotate-button:hover { transform: rotate(360deg); }` },
    { label: 'Tilt', class: 'tilt-button', css:`
    .tilt-button { background-color: #20c997; color: white; border: none;border-radius: 5px; padding: 12px 24px; transition: transform 0.3s ease; }
    .tilt-button:hover { transform: perspective(500px) rotateX(20deg) rotateY(20deg); }` },
    { label: 'Flip', class: 'flip-button', css:`
    .flip-button { background-color: #ff6b6b; color: white; perspective: 1000px; }
    .flip-button:hover { transform: rotateY(180deg); transition: transform 0.3s ease; }` },
    { label: 'Border Slide', class: 'btn-border-slide', css:`
    .btn-border-slide { background-color: #1a1a59; color: white; padding: 12px 24px; cursor: pointer; position: relative; overflow: hidden; transition: border-color 0.3s ease; }
    .btn-border-slide::before { content: ""; position: absolute; top: 0; left: -100%; width: 100%; height: 100%; border: 2px solid #ffffff; transition: left 0.3s ease; }
    .btn-border-slide:hover::before { left: 0; }` },
    { label: 'Corner', class: 'corner-button', css:`
    .corner-button { background-color: #007BFF; color: white; border: none;border-radius: 0; padding: 12px 24px; transition: border-radius 0.3s ease; }
    .corner-button:hover { border-radius: 15px; }` },
    { label: 'Comic', class: 'comic-button', css:`
    .comic-button { background-color: #ff6347; color: #fff; border: 3px solid #000; border-radius: 10px; padding: 12px 24px; font-size: 16px; font-family: 'Comic Sans MS', cursive, sans-serif;position: relative; box-shadow: 5px 5px 0 #000; transition: transform 0.2s, box-shadow 0.2s; }
    .comic-button:hover { transform: translate(-3px, -3px); box-shadow: 8px 8px 0 #000; }` },
    { label: 'Swipe', class: 'swipe-button', css:`
    .swipe-button { color: #fff; border: 3px solid #c266d3; background-image: linear-gradient(30deg, #c266d3 50%, transparent 50%); background-size: 500px; background-repeat: no-repeat;background-position: 0%; transition: background 300ms ease-in-out; }
    .swipe-button:hover { background-position: 100%; color: #c266d3; }` },
    { label: 'Morph', class: 'morph-button', css:`
    .morph-button { background-color: #6c757d; color: white; transition: transform 0.3s ease, border-radius 0.3s ease; }
    .morph-button:hover { transform: scale(1.2); border-radius: 50%; }` },
    { label: 'Ripple', class: 'ripple-button', css:`
    .ripple-button { background-color: #6f42c1; color: white; padding: 12px 24px; font-size: 16px; border: none; border-radius: 5px; overflow: hidden; position: relative; transition: all 0.3s ease-in-out; }
    ...
    { label: 'Scale Bounce', class: 'scale-bounce-btn', css: `
    .scale-bounce-btn { background-color: #6c5ce7; color: white; border: none;transform-origin: center; transition: all 0.2s ease; box-shadow: 0 4px 6px rgba(108, 92, 231, 0.2); }
    .scale-bounce-btn:hover { animation: bounce 0.5s; box-shadow: 0 6px 10px rgba(108, 92, 231, 0.4); }
    @keyframes bounce { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } }` },
    { label: 'Dotted', class: 'dotted-button', css:`
    .dotted-button { background-color: #e83e8c; color: white; padding: 12px 24px; font-size: 16px; border: none; border-radius: 5px; position: relative; }
    ...
    { label: 'C-Button', class: 'c-button', css:`
    .c-button { padding: 1em 2em; border: none; border-radius: 5px; font-weight: bold; letter-spacing: 5px; text-transform: uppercase; cursor: pointer; color: #2c9caf; transition: all 1000ms; font-size: 15px; position: relative; overflow: hidden; outline: 2px solid #2c9caf; }
    ...
    { label: 'Simple2', class: 'button-simple2', css:`
    .button-simple2 { background-color: #f3f7fe; color: #3b82f6; border: none; cursor: pointer; border-radius: 8px; width: 100px; height: 45px; transition: 0.3s; }
    .button-simple2:hover { background-color: #3b82f6; box-shadow: 0 0 0 5px #3b83f65f; color: #fff; }` },
    { label: 'Border Hover', class: 'button-border', css:`
    .button-border { padding: 15px; border-radius: 5px; outline: 2px solid #2835ba; outline-offset: 0px; background: #2835ba; border: 0; color: white; transition: all .1s ease-in-out; cursor: pointer; }
    .button-border:hover { outline-offset: 3px; outline: 3px solid #2835ba; }` },
    { label: 'B Move', class: 'border-move', css:`
    .border-move { position: relative; background: #FBD148; box-sizing: border-box; border: 0; outline: 0; padding: 1rem 3rem; font-size: 1em; cursor: pointer; transition: .25s ease; }
    ...
    { label: 'Brightness', class: 'brightness-button', css:`
    .brightness-button { background: linear-gradient(to top, #30cfd0, #330867); color: white; border-radius: 10px; padding: 10px 20px; }
    .brightness-button:hover { filter: brightness(1.3); }` },
    { label: 'Neon Glow', class: 'neon-glow-btn', css: `
    .neon-glow-btn { background-color: transparent; color: #00FFC6; border: 2px solid #00FFC6; box-shadow: 0 0 10px rgba(0, 255, 198, 0), 0 0 10px rgba(0, 255, 198, 0);transition: all 0.3s ease; text-shadow: 0 0 0px rgba(0, 255, 198, 0); }
    .neon-glow-btn:hover { color: #111; background-color: #00FFC6; box-shadow: 0 0 20px rgba(0, 255, 198, 0.8), 0 0 40px rgba(0, 255, 198, 0.4); text-shadow: 0 0 5px rgba(255, 255, 255, 0.8); }` },
    { label: 'Glass', class: 'glassmorphism-button', css:`
    .glassmorphism-button { background: rgba(255, 255, 255, 0.2); border: 1px solid rgba(255, 255, 255, 0.3); backdrop-filter: blur(40px); color: black; padding: 10px 20px;border-radius: 10px; transition: background 0.3s ease, border 0.3s ease; }
...
<style scoped>
  ...
  .neon-glow-btn {
    ...
    box-shadow: 0 0 10px rgba(0, 255, 198, 0), 0 0 10px rgba(0, 255, 198, 0);
    transition: all 0.3s ease;
    text-shadow: 0 0 0px rgba(0, 255, 198, 0);
  }
  .neon-glow-btn:hover {
    color: #111;
    background-color: #00FFC6;
    box-shadow: 0 0 20px rgba(0, 255, 198, 0.8), 0 0 40px rgba(0, 255, 198, 0.4);
