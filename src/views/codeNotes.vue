<template>
  <v-container class="d-flex flex-column align-center w-100">
    ...
    <v-btn color="primary" dark @click="openNewNoteDialog" class="mb-4"
      >Add +</v-btn
    >
    <v-dialog v-model="showDialog" max-width="600">
      <v-card color="indigo darken-4">
        ...
        <v-card-actions class="white--text">
          <v-spacer></v-spacer>
          <v-btn color="blue-grey darken-3" text @click="closeDialog"
            >Cancel</v-btn
          >
    ...
    <v-row class="w-100" justify="center">
      ...
      >
        <v-card
          class="pa-2 d-flex flex-column align-center color-white"
          :style="{ backgroundColor: note.color }"
        >
          <v-card-title class="text-center text-wrap text-h5">
            {{ note.title }}
          </v-card-title>
    ...
    <v-snackbar
      ...
      timeout="2000"
      color="green"
      class="d-flex justify-center align-end w-10"
    >
      Note saved!
    </v-snackbar>
  </v-container>
...
const saveNote = () => {
  note.value.color =
    languageColors[note.value.language.text] ||
    ""; /* note background color HTML, CSS and JS */
...
onMounted(() => {
  document.body.style.background =
    "linear-gradient(to right, #4F4F92, #2D2D52)";
