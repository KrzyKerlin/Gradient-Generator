<template>
  <v-container class="d-flex flex-column align-center pa-2">
    ...
    <v-card
      elevation="0"
      class="w-100 d-flex flex-column justify-center mt-2"
      ...
      <v-card>
        <v-card-text>
          ...
          <div class="d-flex flex-column align-center">
            <h4
              class="clickable-text text-center my-4"
              @click="copyToClipboard(selectedIcon)"
            >
              {{ selectedIcon }}
            </h4>
            <small v-if="isCopied" class="text-green bold mt-2">
              Icon copied!
            </small>
          </div>
          <v-card-actions>
            <v-btn color="primary" text block @click="closePopup">
              Close
            </v-btn>
          </v-card-actions>
        </v-card-text>
...
onMounted(() => {
  document.body.style.background =
    "linear-gradient(to right, #8987e3, #c4c4e4)";
  loadIcons();
});
