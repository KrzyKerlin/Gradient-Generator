<template>
  <v-container class="d-flex flex-column align-center">
    <BackButton />
    <h1 class="my-4 text-center text-indigo-darken-4">WebP <br /> Converter</h1>    
    <v-btn color="primary" @click="triggerFileInput" class="my-4" size="large">
      <v-icon start>mdi-upload</v-icon>
      Upload Images
    </v-btn>
    <input type="file" multiple @change="loadImages" ref="fileInput" class="d-none" id="fileInput" accept="image/*" />
  
    <v-card v-if="files.length" class="image-container pa-4 mb-4" elevation="3" width="100%" max-width="600px">
        ...
        <v-list-item v-for="(file, index) in files" :key="index" class="mb-2">
          <v-card width="100%" class="file-card">
            <div class="d-flex align-center px-3 py-2">
              <div class="d-flex align-center justify-center bg-grey-lighten-3 rounded overflow-hidden" style="width: 40px; height: 40px;">
                <img v-if="previews[index]" :src="previews[index]" alt="Preview" class="preview-img" />
                <v-icon v-else size="36">mdi-file-image</v-icon>
              </div>
              
              <div class="text-truncate flex-grow-1 text-body-2">{{ file.name }}</div>
      ...
      <v-card-actions v-if="files.length > 1" class="justify-center mt-3">
        <v-btn color="green" size="large" @click="downloadImages" variant="elevated">
