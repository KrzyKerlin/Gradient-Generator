<template>
  <v-container class="d-flex flex-column align-center">
    <BackButton />
    <h1 class="mb-8 text-center text-primary">
      Font <span class="second">Picker</span>
    </h1>
    ...
    <v-row class="my-4" v-if="fonts.length > 0" justify="center">
      ...
      >
        ...
        >
          <v-card-text
            class="text-center d-flex align-center justify-center text-box my-2"
            >{{ userText || "Sample Text" }}
          </v-card-text>
          <div
            v-if="hoverFont === font.family"
            class="font-name text-primary font-weight-medium"
          >
            {{ font.family }}
          </div>
        </v-card>
